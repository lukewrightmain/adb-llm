...
Architecture:
  Phone:60000 (swarm-rpc)
    -> adb forward on Windows PC: WinPC:600XX -> Phone:60000
    -> SSH tunnel from this server: localhost:600XX -> WinPC:600XX

This gives swarm-server (running on this Linux server) localhost endpoints
that reach swarm-rpc on each phone, routed through Windows PC over USB.
"""
...
from cellswarm.core.config import RPC_BASE_LOCAL_PORT, RPC_REMOTE_PORT
from cellswarm.core.device import DeviceInfo
from cellswarm.core.errors import TunnelError
from cellswarm.transport.base import TransportConnection, TransportLayer
from cellswarm.utils.adb import adb_forward, adb_forward_remove, ssh_tunnel
...
class AdbForwardTransport(TransportLayer):
    ...
    Result: localhost:600XX on this server reaches swarm-rpc on the phone.
    """
