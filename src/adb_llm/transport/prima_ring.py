"""Ring topology transport for prima.cpp pipeline-ring parallelism.

prima.cpp uses ZMQ PUSH/PULL sockets in a ring:
...
class RingTopology:
    ...
    def layer_assignment(self, total_layers: int) -> list[int]:
        ...
        prima.cpp requires all layer window values > 0.
        """
...
class PrimaRingTransport:
    """Manage the full ring of tunnels for prima.cpp pipeline parallelism.
    ...
    async def setup_ring(self, devices: list[DeviceInfo]) -> RingTopology:
        ...
        host_node = RingNode(
            ...
            device=None,
            serial="",
            data_bind_port=PRIMA_DATA_PORT,          # 9000
            signal_bind_port=PRIMA_SIGNAL_PORT,       # 10000
            host_reachable_data_port=PRIMA_DATA_PORT,
            host_reachable_signal_port=PRIMA_SIGNAL_PORT,
        )
        ...
        for idx, device in enumerate(devices):
            ...
            node = RingNode(
                ...
                data_bind_port=PRIMA_DATA_PORT + rank,
                signal_bind_port=PRIMA_SIGNAL_PORT + rank,
                # SSH -L local port must match prima.cpp's expected port (data_port + rank)
                host_reachable_data_port=PRIMA_DATA_PORT + rank,
                host_reachable_signal_port=PRIMA_SIGNAL_PORT + rank,
                winpc_fwd_data_port=PRIMA_FORWARD_BASE + idx,
                winpc_fwd_signal_port=PRIMA_FORWARD_BASE + self.SIGNAL_OFFSET + idx,
            )
        ...
        for idx, device in enumerate(devices):
            ...
            phone_next_data = PRIMA_DATA_PORT + next_rank
            phone_next_signal = PRIMA_SIGNAL_PORT + next_rank
    ...
    async def _create_inbound_tunnels(
        ...
        # Data port
        win_fwd_port = PRIMA_FORWARD_BASE + idx
        ...
        # Signal port
        win_fwd_sig = PRIMA_FORWARD_BASE + self.SIGNAL_OFFSET + idx
    ...
    async def _create_outbound_tunnels(
        ...
        """
        # Data outbound
        win_rev_data = PRIMA_SSH_REVERSE_BASE + idx
        ...
        # Signal outbound
        win_rev_sig = PRIMA_SSH_REVERSE_BASE + self.SIGNAL_OFFSET + idx
