"""USB Tethering transport for prima.cpp pipeline-ring parallelism.
...
Topology (same ring as prima_ring.py):
...
from adb_llm.core.config import PRIMA_DATA_PORT, PRIMA_SIGNAL_PORT
...
class UsbTetherTransport:
    """Manage USB tethering network for prima.cpp phone-only ring.
    ...
    async def setup_ring(self, devices: list[DeviceInfo]) -> TetherTopology:
        ...
        for idx, device in enumerate(devices):
            ...
            node = TetherNode(
                ...
                tether_ip=ip,
                data_bind_port=PRIMA_DATA_PORT + idx,
                signal_bind_port=PRIMA_SIGNAL_PORT + idx,
            )
