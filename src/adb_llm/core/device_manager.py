...
from adb_llm.core.config import REMOTE_BASE, REMOTE_RPC_SERVER
from adb_llm.core.device import DeviceInfo, DeviceState
from adb_llm.core.errors import DeviceNotFoundError
from adb_llm.utils.adb import adb_devices, adb_shell
    ...
    async def probe(self, serial: str) -> DeviceInfo:
        ...
        try:
            results = await asyncio.gather(
                ...
                adb_shell(serial, f"ls {REMOTE_RPC_SERVER}", check=False),
            ...
            dev.has_rpc_server = "No such file" not in results[7] and results[7].strip() != ""
            if dev.has_rpc_server:
                dev.rpc_server_path = REMOTE_RPC_SERVER
