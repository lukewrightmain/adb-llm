...
echo "  Output:    $OUTPUT_DIR/rpc-server"
echo ""
...
RPC_BIN=""
for candidate in bin/rpc-server rpc-server examples/rpc/rpc-server; do
    if [ -f "$candidate" ]; then
        RPC_BIN="$candidate"
        break
    fi
...
if [ -z "$RPC_BIN" ]; then
    echo "ERROR: rpc-server binary not found in build directory"
    echo "Contents of build dir:"
    find . -name "rpc-server" -o -name "rpc_server" 2>/dev/null
    exit 1
fi

cp "$RPC_BIN" "$OUTPUT_DIR/rpc-server"

echo ""
echo "========================================="
echo "SUCCESS: $OUTPUT_DIR/rpc-server"
file "$OUTPUT_DIR/rpc-server"
ls -lh "$OUTPUT_DIR/rpc-server"
echo "========================================="
