#!/usr/bin/env bash
# Benchmark cellswarm phone-only ring — NO host compute.
# All inference runs on phones. Rank 0 = first phone (same cellswarm-worker binary with -r 0).
#
# Usage:
#   ./scripts/bench_cellswarm_phoneonly.sh 3                     # 3-phone ring, Q4_0, CPU only
#   ./scripts/bench_cellswarm_phoneonly.sh 5                     # 5-phone ring (sweet spot)
#   ./scripts/bench_cellswarm_phoneonly.sh 5 --fp16              # 5-phone + FP16 activations (halves transfer)
#   ./scripts/bench_cellswarm_phoneonly.sh 5 --tether rndis      # 5-phone + USB tethering (bypass ADB tunnels)
#   ./scripts/bench_cellswarm_phoneonly.sh 5 --fp16 --tether rndis  # FP16 + tethering (best combo)
#   ./scripts/bench_cellswarm_phoneonly.sh 5 --bottom            # Use bottom phones (avoid other agents)
#   ./scripts/bench_cellswarm_phoneonly.sh 5 --spec              # 5-phone + speculative (rank 0 runs spec binary)
#   ./scripts/bench_cellswarm_phoneonly.sh 5 --spec --draft-max 16  # Tune draft batch size
#   ./scripts/bench_cellswarm_phoneonly.sh 5 --sweep             # Sweep 3,4,5,6 phone configs
#   MODEL=Q4_K_M ./scripts/bench_cellswarm_phoneonly.sh 3        # Use Q4_K_M instead of Q4_0
#
# Prerequisites:
#   - Phones have cellswarm-worker + Q4_0 model deployed (./scripts/deploy_model.sh)
#   - For speculative: cellswarm-worker-spec + draft model on rank 0 (deploy_model.sh --all)
...
DRAFT_REMOTE="/data/local/tmp/cellswarm/models/deepseek-coder-1.3b-instruct.Q4_0.gguf"
...
cleanup() {
    ...
    for SERIAL in "${ALL_PHONES[@]}"; do
        ssh -o ConnectTimeout=5 "$SSH_HOST" "$ADB -s $SERIAL shell pkill -9 cellswarm-worker" 2>/dev/null || true
        ssh -o ConnectTimeout=5 "$SSH_HOST" "$ADB -s $SERIAL shell pkill -9 cellswarm-worker-spec" 2>/dev/null || true
    ...
    pkill -f cellswarm-worker 2>/dev/null || true
...
# Start cellswarm-workers on all phones
...
start_phone_workers() {
    ...
    echo "Starting cellswarm-workers on $n_phones phones (act_quant=$act_q, tether=$tether)..."
    ...
    for idx in $(seq 0 $((n_phones - 1))); do
        ...
        local script_local="/tmp/cellswarm-phoneonly-rank${rank}.sh"
        local script_win="C:\\Users\\Lukio-4090\\cellswarm-phoneonly-rank${rank}.sh"
        local script_phone="/data/local/tmp/cellswarm-start.sh"
        ...
        # Rank 0 with speculative mode uses cellswarm-worker-spec
        if [ "$rank" -eq 0 ] && [ "$spec" = "true" ]; then
            cat > "$script_local" << ENDSCRIPT
#!/system/bin/sh
cd /data/local/tmp
taskset f0 ./cellswarm/bin/cellswarm-worker-spec \
  -m $MODEL_REMOTE \
...
#!/system/bin/sh
cd /data/local/tmp
taskset f0 ./cellswarm/bin/cellswarm-worker \
  -m $MODEL_REMOTE \
  --world $n_phones --rank 0 \
  --master $master_ip --next $next_ip \
  --data-port $DATA_PORT --signal-port $SIGNAL_PORT \
...
ENDSCRIPT
    ...
    for idx in $(seq 1 $((n_phones - 1))); do
        ...
        if [ -n "$pid" ]; then
            echo "  $serial (rank $idx): cellswarm-worker running (pid $pid)"
        else
            echo "  $serial (rank $idx): WARNING — cellswarm-worker NOT running!"
    ...
    ssh -o ConnectTimeout=10 "$SSH_HOST" "$ADB -s $rank0_serial shell sh /data/local/tmp/cellswarm-start.sh" 2>/dev/null
...
wait_and_report() {
    ...
    while [ "$elapsed" -lt "$max_wait" ]; do
        ...
        local pid
        pid=$(ssh -o ConnectTimeout=5 "$SSH_HOST" "$ADB -s $rank0_serial shell pidof cellswarm-worker" 2>/dev/null || true)
        # Also check for speculative binary
        if [ -z "$pid" ]; then
            pid=$(ssh -o ConnectTimeout=5 "$SSH_HOST" "$ADB -s $rank0_serial shell pidof cellswarm-worker-spec" 2>/dev/null || true)
        fi
        ...
        local has_perf
        has_perf=$(ssh -o ConnectTimeout=5 "$SSH_HOST" "$ADB -s $rank0_serial shell grep -c llama_perf /data/local/tmp/cellswarm-worker.log 2>/dev/null" || echo "0")
    ...
    echo ""
    echo "--- Rank 0 Log ---"
    ssh -o ConnectTimeout=10 "$SSH_HOST" "$ADB -s $rank0_serial shell cat /data/local/tmp/cellswarm-worker.log" 2>/dev/null | tee "$log_file"
    ...
    for idx in $(seq 1 $((n_phones - 1))); do
        ...
        ssh -o ConnectTimeout=10 "$SSH_HOST" "$ADB -s $serial shell tail -40 /data/local/tmp/cellswarm-worker.log" 2>/dev/null || echo "  (no log)"
    done
...
run_benchmark() {
    ...
    local log_file="/tmp/cellswarm-phoneonly-${n_phones}phone"
    if [ "$ACT_QUANT" = "fp16" ]; then
...
if $SWEEP_MODE; then
    ...
    for n in 3 4 5 6; do
        echo "  /tmp/cellswarm-phoneonly-${n}phone-${MODEL_QUANT}.log"
    done
    echo "============================================"
else
