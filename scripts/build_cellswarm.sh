#!/usr/bin/env bash
# Cross-compile cellswarm for aarch64 Android + native Linux host.
#
# Produces:
...
#
# Usage: ./scripts/build_cellswarm.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
SWARM_DIR="$PROJECT_DIR/vendor/cellswarm"
...
# Validate sources
for dir in "$SWARM_DIR" "$LIBZMQ_DIR" "$CPPZMQ_DIR"; do
    if [ ! -d "$dir" ]; then
...
echo "============================================"
echo " cellswarm build — ARM64 Android + x86_64 host"
echo "============================================"
echo "  cellswarm: $SWARM_DIR"
echo "  NDK:       $NDK"
...
# ==========================================
# Stage B: cellswarm-worker for ARM64 Android
# ==========================================
...
if [ "$ENABLE_VULKAN" = "1" ]; then
    echo ""
    echo ">>> Stage B: Building cellswarm-worker (ARM64 Android + Vulkan)..."
else
    echo ""
    echo ">>> Stage B: Building cellswarm-worker (ARM64 Android, CPU only)..."
fi

SWARM_BUILD_ANDROID="$BUILD_BASE/cellswarm-android"
rm -rf "$SWARM_BUILD_ANDROID/CMakeCache.txt" "$SWARM_BUILD_ANDROID/CMakeFiles"
mkdir -p "$SWARM_BUILD_ANDROID"
cd "$SWARM_BUILD_ANDROID"
...
# Base flags (CPU-only)
WORKER_CPPFLAGS="-DSWARM_DEBUG -I${ZMQ_INSTALL_ANDROID}/include -I${CPPZMQ_DIR} -DGGML_USE_CPU -D__ANDROID__"
...
if [ "$ENABLE_VULKAN" = "1" ]; then
    ...
    VK_SHADERS_HPP="$SWARM_DIR/ggml/src/ggml-vulkan-shaders.hpp"
    VK_SHADERS_CPP="$SWARM_DIR/ggml/src/ggml-vulkan-shaders.cpp"
    ...
    if [ ! -f "$VK_SHADERS_GEN" ]; then
        ...
        g++ -std=c++11 -O2 -o "$VK_SHADERS_GEN" \
            "$SWARM_DIR/ggml/src/vulkan-shaders/vulkan-shaders-gen.cpp" -lpthread
    fi
    if [ ! -f "$GLSLC" ]; then
    ...
    "$VK_SHADERS_GEN" \
        --glslc "$GLSLC" \
        --input-dir "$SWARM_DIR/ggml/src/vulkan-shaders" \
        --target-hpp "$VK_SHADERS_HPP" \
        --target-cpp "$VK_SHADERS_CPP"

    cp "$VK_SHADERS_GEN" "$SWARM_DIR/vulkan-shaders-gen"
    chmod +x "$SWARM_DIR/vulkan-shaders-gen"
    sleep 1
...
if [ -f "$SWARM_DIR/llama-cli" ]; then
    cp "$SWARM_DIR/llama-cli" "$OUTPUT_DIR/cellswarm-worker"
    echo "  cellswarm-worker built: $OUTPUT_DIR/cellswarm-worker"
    file "$OUTPUT_DIR/cellswarm-worker"
else
    # Try alternate location
    FOUND=$(find "$SWARM_DIR" -name "llama-cli" -type f 2>/dev/null | head -1)
    if [ -n "$FOUND" ]; then
        cp "$FOUND" "$OUTPUT_DIR/cellswarm-worker"
        echo "  cellswarm-worker built: $OUTPUT_DIR/cellswarm-worker"
    else
...
# ==========================================
# Stage B2: cellswarm-worker-spec for ARM64 Android (speculative decoding)
# ==========================================
...
echo ""
echo ">>> Stage B2: Building cellswarm-worker-spec (ARM64 Android, speculative decoding)..."
...
if [ -f "$SWARM_DIR/llama-speculative" ]; then
    cp "$SWARM_DIR/llama-speculative" "$OUTPUT_DIR/cellswarm-worker-spec"
    echo "  cellswarm-worker-spec built: $OUTPUT_DIR/cellswarm-worker-spec"
    file "$OUTPUT_DIR/cellswarm-worker-spec"
else
    FOUND=$(find "$SWARM_DIR" -name "llama-speculative" -type f 2>/dev/null | head -1)
    if [ -n "$FOUND" ]; then
        cp "$FOUND" "$OUTPUT_DIR/cellswarm-worker-spec"
        echo "  cellswarm-worker-spec built: $OUTPUT_DIR/cellswarm-worker-spec"
    else
        echo "WARNING: llama-speculative (ARM64) binary not found — skipping cellswarm-worker-spec"
    fi
fi

# Clean cellswarm build artifacts for host build
make clean 2>/dev/null || true
unset CC CXX AR RANLIB

# ==========================================
# Stage C: cellswarm-host for x86_64 Linux
# ==========================================
echo ""
echo ">>> Stage C: Building cellswarm-host (x86_64 Linux)..."
...
# Build cellswarm-host (llama-cli with HiGHS for rank 0)
cd "$SWARM_DIR"
make clean 2>/dev/null || true

HOST_MAKE_FLAGS=(
    CPPFLAGS="-DSWARM_DEBUG ${ZMQ_CFLAGS} -I${CPPZMQ_DIR} ${HIGHS_CFLAGS} -DUSE_HIGHS"
...
if [ -f "$SWARM_DIR/llama-cli" ]; then
    cp "$SWARM_DIR/llama-cli" "$OUTPUT_DIR/cellswarm-host"
    echo "  cellswarm-host built: $OUTPUT_DIR/cellswarm-host"
    file "$OUTPUT_DIR/cellswarm-host"
else
    FOUND=$(find "$SWARM_DIR" -name "llama-cli" -type f 2>/dev/null | head -1)
    if [ -n "$FOUND" ]; then
        cp "$FOUND" "$OUTPUT_DIR/cellswarm-host"
        echo "  cellswarm-host built: $OUTPUT_DIR/cellswarm-host"
    else
...
# ==========================================
# Stage D: cellswarm-host-spec (speculative decoding host)
# ==========================================
echo ""
echo ">>> Stage D: Building cellswarm-host-spec (x86_64 Linux, speculative decoding)..."
...
if [ -f "$SWARM_DIR/llama-speculative" ]; then
    cp "$SWARM_DIR/llama-speculative" "$OUTPUT_DIR/cellswarm-host-spec"
    echo "  cellswarm-host-spec built: $OUTPUT_DIR/cellswarm-host-spec"
    file "$OUTPUT_DIR/cellswarm-host-spec"
else
    FOUND=$(find "$SWARM_DIR" -name "llama-speculative" -type f 2>/dev/null | head -1)
    if [ -n "$FOUND" ]; then
        cp "$FOUND" "$OUTPUT_DIR/cellswarm-host-spec"
        echo "  cellswarm-host-spec built: $OUTPUT_DIR/cellswarm-host-spec"
    else
        echo "WARNING: llama-speculative binary not found — skipping cellswarm-host-spec"
    fi
fi
