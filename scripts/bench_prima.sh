#!/usr/bin/env bash
# Quick benchmark for prima.cpp pipeline-ring inference.
#
# Usage:
#   ./scripts/bench_prima.sh 2        # 2-node (1 phone), CPU only
#   ./scripts/bench_prima.sh 2 20     # 2-node, 20 GPU layers on phones (Vulkan)
#   ./scripts/bench_prima.sh 4        # 4-node (3 phones), CPU only
#   ./scripts/bench_prima.sh 4 10     # 4-node, 10 GPU layers on phones
#
# Environment variables:
#   ACT_QUANT=fp16  — enable FP16 activation quantization (halves transfer size)
#
# Prerequisites: phones have prima-worker + model already deployed.
...
PRIMA_HOST="$PROJECT_DIR/bin/prima-host"
...
echo "============================================"
echo " prima.cpp benchmark — ${WORLD_SIZE}-node ring"
echo "============================================"
echo "  Phones: ${PHONES[*]}"
if [ "$NGL" -gt 0 ]; then
...
cleanup() {
    echo "Cleaning up..."
    # Kill prima-workers and remove ADB tunnels
    ...
    # Kill SSH tunnels on prima ports (9001+, 10001+, 58xxx, 59xxx)
...
for idx in $(seq 0 $((N_PHONES - 1))); do
    ...
    # Local port MUST match what prima.cpp expects (data_port + rank)
...
time "$PRIMA_HOST" \
    ...
    -n 128 2>&1 | tee /tmp/prima-bench-${WORLD_SIZE}node.log

echo ""
echo "============================================"
echo " Benchmark complete. Log: /tmp/prima-bench-${WORLD_SIZE}node.log"
echo "============================================"
