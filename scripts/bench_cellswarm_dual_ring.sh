...
# Each ring runs its own cellswarm-worker-spec instance independently.
#
# Usage:
#   ./scripts/bench_cellswarm_dual_ring.sh              # 2×10 phones, draft-max=24
#   ./scripts/bench_cellswarm_dual_ring.sh --draft-max 16
#   RING_SIZE=8 ./scripts/bench_cellswarm_dual_ring.sh  # 2×8 phones
...
cleanup() {
    ...
    for ip in "${ALL_PHONES[@]}"; do
        adb_shell "$ip" "pkill -9 cellswarm-worker; pkill -9 cellswarm-worker-spec" 2>/dev/null || true
    done
    sleep 1
}
...
wait_for_ring() {
    ...
    while [ "$elapsed" -lt "$max_wait" ]; do
        ...
        elapsed=$((elapsed + 5))
        local pid
        pid=$(adb_shell "$rank0_ip" "pidof cellswarm-worker 2>/dev/null || pidof cellswarm-worker-spec 2>/dev/null" 2>/dev/null | tr -d '\r' || true)
        if [ -z "$pid" ]; then
    ...
    adb_shell "$rank0_ip" "cat /data/local/tmp/cellswarm-worker.log" 2>/dev/null > "$log_file"
...
# Verify non-rank-0 workers (rank 0 uses cellswarm-worker-spec, started later)
all_ok=true
for idx in $(seq 1 $((RING_SIZE - 1))); do
    ...
    if [ -z "$pid" ]; then
        ...
        adb_shell "$ip" "tail -10 /data/local/tmp/cellswarm-worker.log" 2>/dev/null || true
        all_ok=false
    else
...
LOG_A="/tmp/cellswarm-dual-ringA-${RING_SIZE}phone-spec-d${DRAFT_MAX}-${MODEL_QUANT}.log"
LOG_B="/tmp/cellswarm-dual-ringB-${RING_SIZE}phone-spec-d${DRAFT_MAX}-${MODEL_QUANT}.log"
...
while [ "$elapsed" -lt "$max_wait" ]; do
    ...
    if ! $a_done; then
        pid_a=$(adb_shell "${RING_A_PHONES[0]}" "pidof cellswarm-worker-spec 2>/dev/null" 2>/dev/null | tr -d '\r' || true)
        [ -z "$pid_a" ] && a_done=true && echo "  Ring A finished (${elapsed}s)"
    fi
    if ! $b_done; then
        pid_b=$(adb_shell "${RING_B_PHONES[0]}" "pidof cellswarm-worker-spec 2>/dev/null" 2>/dev/null | tr -d '\r' || true)
...
# Fetch logs
adb_shell "${RING_A_PHONES[0]}" "cat /data/local/tmp/cellswarm-worker.log" 2>/dev/null > "$LOG_A"
adb_shell "${RING_B_PHONES[0]}" "cat /data/local/tmp/cellswarm-worker.log" 2>/dev/null > "$LOG_B"
