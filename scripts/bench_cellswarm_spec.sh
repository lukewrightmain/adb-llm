#!/usr/bin/env bash
# Benchmark cellswarm with speculative decoding (draft model on host).
#
# Usage:
#   ./scripts/bench_cellswarm_spec.sh 2                    # 2-node, default (draft-max=8)
#   ./scripts/bench_cellswarm_spec.sh 2 8 42               # 2-node, draft-max=8, host=42 layers
#   ./scripts/bench_cellswarm_spec.sh 4 16 20              # 4-node, draft-max=16, host=20 layers
#   DRAFT_MODEL=~/models/other.gguf ./scripts/bench_cellswarm_spec.sh 2
#
# Prerequisites:
#   - bin/cellswarm-host-spec built (./scripts/build_cellswarm.sh)
#   - Draft model downloaded (deepseek-coder-1.3b-instruct.Q4_K_M.gguf)
#   - Phones have cellswarm-worker + target model deployed
...
SWARM_HOST_SPEC="$PROJECT_DIR/bin/cellswarm-host-spec"
...
# Validate
if [ ! -f "$SWARM_HOST_SPEC" ]; then
    echo "ERROR: $SWARM_HOST_SPEC not found. Run ./scripts/build_cellswarm.sh first."
    exit 1
fi
...
echo "============================================"
echo " cellswarm SPECULATIVE benchmark — ${WORLD_SIZE}-node ring"
echo "============================================"
...
cleanup() {
    ...
    pkill -f cellswarm-host-spec 2>/dev/null || true
...
# --- Set up tunnels (same as bench_cellswarm.sh) ---
...
LOG_FILE="/tmp/cellswarm-spec-${WORLD_SIZE}node-d${DRAFT_MAX}-h${HOST_LAYERS}.log"
