...
#   - cellswarm-worker + model deployed to phones
...
SEED=42
NO_INTERLEAVE=false
BATCH_PIPELINE="${SWARM_BATCH_PIPELINE:-1}"
FLASH_ATTN=false
CTX_SIZE=512
THREADS=4
TASKSET="f0"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --master)         MASTER_MODE=true; SPEC_MODE=true ;;
...
cleanup() {
    ...
    for ip in "${ALL_PHONES[@]}"; do
        adb_shell "$ip" "pkill -9 cellswarm-worker; pkill -9 cellswarm-worker-spec; pkill -9 cellswarm-master" 2>/dev/null || true
    done
    sleep 1
    echo "Cleanup done."
}
...
start_phone_workers() {
    ...
    echo "Starting cellswarm-workers on $n_phones phones (direct IP)..."

    local profile_prefix=""
    if [ "$PROFILE_MODE" = "true" ]; then
        profile_prefix="SWARM_DECODE_PROFILE=1 "
        echo "  (decode profiling ENABLED)"
    fi
    ...
    for idx in $(seq 1 $((n_phones - 1))); do
        ...
        adb_shell "$ip" "sh -c '
cd /data/local/tmp
${profile_prefix}taskset $TASKSET ./cellswarm/bin/cellswarm-worker \
  -m $MODEL_REMOTE \
...
'" 2>/dev/null
    ...
    local binary="cellswarm-worker"
    local extra_flags=""
    if [ "$spec" = "true" ]; then
        binary="cellswarm-worker-spec"
        extra_flags="--model-draft $DRAFT_REMOTE --draft-max $DRAFT_MAX"
    fi

    local env_prefix="${profile_prefix}SWARM_BATCH_PIPELINE=$BATCH_PIPELINE "
    if [ "$NO_INTERLEAVE" = "true" ]; then
        env_prefix="${env_prefix}SWARM_NO_INTERLEAVE=1 "
    ...
    if [ "$MASTER_MODE" = "true" ]; then
        # Master mode: use cellswarm-master (HTTP server) for rank 0
        binary="cellswarm-master"
        local master_port=8080

        echo ""
        echo "Starting rank 0 (MASTER) on $rank0_ip (next=$next_ip, port=$master_port)..."
        adb_shell "$rank0_ip" "sh -c '
cd /data/local/tmp
${env_prefix}taskset $TASKSET ./cellswarm/bin/$binary \
  ...
  --no-mmap --prefetch $fa_flag $EXTRA_FLAGS \
  --host 0.0.0.0 --port $master_port -np 1 \
  > /data/local/tmp/cellswarm-worker.log 2>&1 &
'" 2>/dev/null
        echo "  Master started on port $master_port. Waiting for model load..."
        sleep 30

        # Send benchmark prompt via curl
        echo "  Sending benchmark prompt via HTTP..."
        curl -s -X POST "http://${rank0_ip}:${master_port}/v1/chat/completions" \
            -H "Content-Type: application/json" \
            -d "{\"messages\":[{\"role\":\"user\",\"content\":\"Write a Python function that computes the Fibonacci sequence efficiently using dynamic programming.\"}],\"stream\":false,\"n_predict\":${N_TOKENS},\"seed\":${SEED}}" \
            > /tmp/cellswarm-master-bench-response.json 2>/dev/null &
        echo "  Benchmark request sent."
    else
        echo ""
...
wait_and_report() {
    ...
    while [ "$elapsed" -lt "$max_wait" ]; do
        ...
        local pid
        pid=$(adb_shell "$rank0_ip" "pidof cellswarm-worker 2>/dev/null || pidof cellswarm-worker-spec 2>/dev/null || pidof cellswarm-master 2>/dev/null" 2>/dev/null | tr -d '\r' || true)

        if [ "$MASTER_MODE" = "true" ]; then
            # In master mode, check if curl got a response
            if [ -f /tmp/cellswarm-master-bench-response.json ] && \
               [ -s /tmp/cellswarm-master-bench-response.json ]; then
                echo "  Master benchmark response received (${elapsed}s elapsed)."
                done=true
                break
            fi
        else
        ...
        local has_perf
        has_perf=$(adb_shell "$rank0_ip" "grep -c llama_perf /data/local/tmp/cellswarm-worker.log 2>/dev/null" | tr -d '\r\n' || echo "0")
    ...
    adb_shell "$rank0_ip" "cat /data/local/tmp/cellswarm-worker.log" 2>/dev/null | tee "$log_file"
    ...
    for idx in $(seq 1 $((n_phones - 1))); do
        ...
        adb_shell "$ip" "tail -30 /data/local/tmp/cellswarm-worker.log" 2>/dev/null || echo "  (no log)"
    done

    echo ""
    echo "--- Key Metrics ---"
    grep -E "(speed:|tok/s|n_draft|n_predict|n_drafted|n_accept|accept|eval time|sample time|total time|DECODE-PROFILE)" "$log_file" 2>/dev/null || echo "  No metrics found."
...
run_benchmark() {
    ...
    local log_file="/tmp/cellswarm-ethernet-${n_phones}phone"
...
if $SPEC_SWEEP_MODE; then
    ...
    for dm in 8 16 24 32; do
        echo "  /tmp/cellswarm-ethernet-${N_PHONES}phone-spec-d${dm}-${MODEL_QUANT}.log"
    done
    echo "============================================"
    ...
    for n in 4 5 8 10 15 20; do
        ...
        echo "  /tmp/cellswarm-ethernet-${n}phone-${suffix}.log"
    done
    echo "============================================"
else
